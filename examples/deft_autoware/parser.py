...
class DeFT_Autoware_Parser:
    def __init__(self, output_directory: pathlib.Path, test_id: Optional[str] = None):
        ...
        self.lastTFMessage: Optional[TFMessage] = None
    ...
    def parse_record_file(self, file_path: pathlib.Path):
        record = Record(file_path)
        for topic, msg, t in record.read_messages():
            if topic == '/tf':
                self.onTFMessage(msg)
            elif topic == '/map/vector_map':
                self.onHDMapBin(msg)
            elif topic == '/perception/object_recognition/objects':
                self.onPredictedObjects(msg)
            elif topic == '/perception/obstacle_segmentation/pointcloud':
                self.onPointCloud2(msg)
            elif topic == '/localization/kinematic_state':
                self.onOdometry(msg)
            elif topic == '/localization/acceleration':
                self.onAccelWithCovarianceStamped(msg)
            elif topic == '/perception/occupancy_grid_map/map':
                self.onOccupancyGrid(msg)
            elif (
                topic
                ...
                self.onPathWithLaneId(msg)
            elif topic == '/perception/traffic_light_recognition/traffic_signals':
                self.onTrafficSignalArray(msg)
            elif topic == '/planning/scenario_planning/max_velocity_default':
                self.onVelocityLimit(msg)
            elif topic == '/perception/virtual_traffic_light_states':
                self.onVirtualTrafficLightStateArray(msg)
            else:
                pass
    ...
    def onHDMapBin(self, msg):
        ...
        if self.test_id is None:
            self.test_id = f'test_{msg.header.stamp.sec}_{msg.header.stamp.nanosec}'

        logger.info(f'New scenario started. Test ID: {self.test_id}')

    def onTFMessage(self, msg):
        self.lastTFMessage = msg
    ...
    def onPath(self, msg):
        ...
        write_frame_to_directory(
            ...
            f'{self.output_directory}/{self.test_id}/{len(self.aggregatedFrames) - 1}',
            False,
        )
...
if __name__ == '__main__':
    record_file_path = (
        '/DeFT_autoware/data/sample_scenarios/e8f16ee1-f260-4891-8ccd-ef3ff9a61fcd_0_0'
    )
    parser = DeFT_Autoware_Parser(pathlib.Path('/DeFT_autoware/data/output'))
    parser.parse_record_file(pathlib.Path(record_file_path))
