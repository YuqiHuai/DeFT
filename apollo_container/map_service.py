...
from apollo_container.kdtree import KDTree, KDTreeParams
...
from apollo_modules.modules.map.proto.map_lane_pb2 import Lane, LaneBoundary
from apollo_modules.modules.map.proto.map_overlap_pb2 import Overlap
...
class MapService:
    ...
    def load_map_from_file(self, filename: str, load_only: bool):
        map = Map()
        with open(filename, 'rb') as fp:
            map.ParseFromString(fp.read())
        self.load_map_from_proto(map, load_only)

    def load_map_from_proto(self, map: Map, load_only: bool):
        obj_table_map = [
